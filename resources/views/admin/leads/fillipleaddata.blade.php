...
@section('content')
    <div class="p-6 space-y-6">
        ...
        <form method="GET" action="{{ route('leadedata') }}">
                ...
                <table class="w-full">
                    <thead>
                        <tr class="bg-gray-50 border-b border-gray-200">
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Quote Details</th>
                                ...
                                if (!empty($lead['message'])) {
                                    ...
                                    foreach ($lines as $line) {
                                        ...
                                        if (strpos($line, '•') !== false) {
                                            ...
                                            $service = trim(str_replace('•', '', $line));
                                            if (!empty($service) && strpos($service, '—') !== false) {
                                                $parts = explode('—', $service);
                                                if (count($parts) >= 2) {
                                                    $serviceName = trim($parts[0]);
                                                    $servicePrice = trim($parts[1] ?? '');
                                                    $services[] = [
                                                        'name' => $serviceName,
                                                        'price' => $servicePrice
                                                    ];
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <span class="px-2 py-1 text-xs font-medium bg-purple-100 text-purple-800 rounded-full">
                                        {{ ucfirst(str_replace('-', ' ', $lead['source'] ?? 'unknown')) }}
                                    </span>
                                </td>
                                        ...
                                        <div class="max-w-xs">
                                            ...
                                            @if(count($services) > 2)
                                                <span class="text-xs text-blue-600 cursor-pointer hover:underline"
                                                      onclick="showFullServices({{ json_encode($services) }})">
                                                    +{{ count($services) - 2 }} more
                                                </span>
                                            @endif
                                        </div>
                                    @else
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    @if(!empty($lead['budget']) && strpos($lead['budget'], '/mo') !== false)
                                        <span class="text-xs text-gray-500 block">Monthly</span>
                                    @elseif(!empty($lead['budget']) && strpos($lead['budget'], 'incl. GST') !== false)
                                ...
                                <td class="px-6 py-4 text-sm text-gray-500">
                                    {{ isset($lead['created_at']) ? \Carbon\Carbon::parse($lead['created_at'])->format('M d, Y') : '-' }}
                                </td>
                            </tr>
                        @empty
                            <tr>
    ...
    <!-- Modal for showing full services -->
    <div id="servicesModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
        <div class="bg-white rounded-xl max-w-md w-full mx-4 p-6">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-semibold text-gray-900">Selected Services</h3>
    ...
    <style>
        ...
        #servicesModal:not(.hidden) {
            display: flex !important;
        }
    </style>

    <script>
        function showFullServices(services) {
