            ...
            <div
                ...
                <div class="p-4 border-b border-gray-100 flex justify-between items-start">
                    <div class="flex items-center space-x-3">
                        <div
                            class="h-12 w-12 flex-shrink-0 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center">
            ...
            ">
                ...
                <div class="p-4 space-y-3">
                    ...
                    <div class="bg-indigo-50 rounded-lg p-3 border border-indigo-100">
                        <div class="flex items-start space-x-2">
                            <svg class="w-4 h-4 text-indigo-500 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-700 line-clamp-3">
                                    {{ $task->task_description ?? 'No description provided' }}</p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="border-t border-gray-100 pt-3">
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                <div class="flex items-center space-x-2">
                                    ...
                                    <span
                                        class="text-sm text-gray-600 truncate">{{ $task->leaddata->email ?? 'N/A' }}</span>
                                </div>
                                <div class="flex items-center space-x-2">
                                    <svg class="w-4 h-4 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                            ...
                            @if ($task->leaddata->country || $task->leaddata->city)
                                <div class="flex items-center space-x-2 mt-1">
                                    ...
                                    <span class="text-xs text-gray-600">
                                        @if ($task->leaddata->city)
                                            {{ $task->leaddata->city }},
                                        @endif
                                        @if ($task->leaddata->state)
                                            {{ $task->leaddata->state }},
                                        @endif
                                        @if ($task->leaddata->country)
                                            {{ $task->leaddata->country }}
                                        @endif
                                    </span>
                                </div>
                            @endif
                        ...
                        ">
                                ...
                                @if ($task->leaddata->budget)
                                    <span class="text-xs text-green-600 font-medium">
                                        ...
                                        @if ($task->leaddata->budget_type)
                                            ({{ $task->leaddata->budget_type }})
                                        @endif
                                    </span>
                                @endif
                            </div>
                            ...
                            @if ($task->leaddata->lead_source)
                                <div class="flex items-center space-x-2 mt-1">
                                    <svg class="w-4 h-4 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9.1-1.645M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                                    </svg>
                                    <span class="text-xs text-gray-600">Source:
                                        {{ ucfirst(str_replace('_', ' ', $task->leaddata->lead_source)) }}</span>
                                </div>
                            @endif
                        </div>
                    @endif
                ...
                <div class="px-4 py-3 bg-gray-50 border-t border-gray-100 flex justify-between items-center">
                    ...
                    <div class="flex space-x-3">
                        <a href=""
                            class="text-blue-600 hover:text-blue-800 transition-colors" title="View Task">
                        ...
                        <a href=""
                            class="text-indigo-600 hover:text-indigo-800 transition-colors" title="Edit Task">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <button onclick="deleteTask({{ $task->id }})"
                            class="text-red-600 hover:text-red-800 transition-colors" title="Delete Task">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        @if ($task->leaddata)
                            <a href=""
                                class="text-green-600 hover:text-green-800 transition-colors" title="View Lead">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="bg-white rounded-xl shadow-md p-12 text-center">
                    <div class="flex flex-col items-center">
                        ...
                        <div class="mt-6 flex flex-col sm:flex-row gap-3">
                            <a href=""
                                class="px-6 py-2.5 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-all shadow-md hover:shadow-lg">
                                Go to Dashboard
                            </a>
                        </div>
                    </div>
