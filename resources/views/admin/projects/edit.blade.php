...
@php
    ...
    $statusColors = [
        ...
        'ongoing' => ['bg' => 'bg-blue-100', 'text' => 'text-blue-700', 'icon' => 'fa-spinner', 'label' => 'Ongoing'],
...
<div class="min-h-screen bg-gradient-to-br from-slate-50 via-gray-100 to-orange-50/30 py-8 px-4 sm:px-6 lg:px-8">
    <div class="max-w-6xl mx-auto">
        ...
        <div class="fixed inset-0 overflow-hidden pointer-events-none">
            <div class="absolute top-20 right-10 w-72 h-72 bg-orange-200 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-float"></div>
            <div class="absolute bottom-20 left-10 w-96 h-96 bg-blue-200 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-float" style="animation-delay: 2s;"></div>
            <div class="absolute top-1/2 right-1/4 w-64 h-64 bg-purple-200 rounded-full filter blur-3xl opacity-10 animate-float" style="animation-delay: 4s;"></div>
        </div>

        {{-- Header Section --}}
        <div class="relative mb-8 animate-fade-up">
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden card-hover">
                <div class="absolute top-0 right-0 w-64 h-64 bg-gradient-to-br from-orange-500 to-orange-600 opacity-10 rounded-full transform translate-x-32 -translate-y-32"></div>
                <div class="relative px-6 py-6 md:px-8 md:py-8">
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        <div class="flex items-center gap-4">
                            <div class="w-16 h-16 bg-gradient-to-br from-orange-500 to-orange-600 rounded-2xl flex items-center justify-center shadow-lg transform -rotate-3 hover:rotate-0 transition-all duration-300">
                        ...
                        <div class="flex items-center gap-3">
                            <div class="status-badge px-4 py-2 {{ $currentStatus['bg'] }} rounded-full">
                                ...
                                <span class="text-sm font-semibold {{ $currentStatus['text'] }}">Current: {{ $currentStatus['label'] }}</span>
                            </div>
        ...
        <form action="{{ route('project.update', $project->id) }}" method="POST" class="animate-fade-up" style="animation-delay: 0.1s">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <i class="fas fa-folder-open absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400"></i>
                                    </div>
                                    @error('name')
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <i class="fas fa-chart-simple absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400"></i>
                                    </div>
                                    @error('status')
                        ...
                        <div class="p-6">
                            ...
                            <div class="mt-4 p-3 bg-orange-50 rounded-xl">
                                <div class="flex items-center gap-2 text-xs text-orange-700">
                                    ...
                                    <span>Tip: Add modules like "Frontend", "Backend", "Database", "API Integration", etc.</span>
                                </div>
                            </div>
                ...
                <div class="lg:col-span-1 space-y-6">
                    ...
                    {{-- Priority Card --}}
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100 card-hover sticky top-6">
                        <div class="px-6 py-4 bg-gradient-to-r from-orange-50 to-amber-50 border-b border-orange-100">
                            <h3 class="font-bold text-gray-800 flex items-center gap-2">
                        ...
                        <div class="p-6">
                            <div class="grid grid-cols-3 gap-2">
                                <label class="cursor-pointer">
                                    <input type="radio" name="priority" value="low" class="hidden peer" {{ old('priority', $project->priority) == 'low' ? 'checked' : '' }}>
                                    <div class="border-2 border-gray-200 rounded-xl p-3 text-center peer-checked:border-green-500 peer-checked:bg-green-50 transition-all duration-200">
                                        <i class="fas fa-arrow-down text-green-500 text-xl"></i>
                                        <p class="text-xs font-medium mt-1 text-gray-600">Low</p>
                                    </div>
                                </label>
                                <label class="cursor-pointer">
                                    <input type="radio" name="priority" value="medium" class="hidden peer" {{ old('priority', $project->priority) == 'medium' ? 'checked' : '' }}>
                                    <div class="border-2 border-gray-200 rounded-xl p-3 text-center peer-checked:border-orange-500 peer-checked:bg-orange-50 transition-all duration-200">
                                        <i class="fas fa-minus text-orange-500 text-xl"></i>
                                        <p class="text-xs font-medium mt-1 text-gray-600">Medium</p>
                                    </div>
                                </label>
                                <label class="cursor-pointer">
                                    <input type="radio" name="priority" value="high" class="hidden peer" {{ old('priority', $project->priority) == 'high' ? 'checked' : '' }}>
                                    <div class="border-2 border-gray-200 rounded-xl p-3 text-center peer-checked:border-red-500 peer-checked:bg-red-50 transition-all duration-200">
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100 card-hover">
                        ...
                        <div class="p-6 space-y-5">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input type="datetime-local" name="end_date"
                                        ...
                                        class="w-full pl-12 pr-4 py-3 border-2 border-gray-200 rounded-xl focus:border-orange-400 transition-all duration-200">
                                    <i class="fas fa-calendar-week absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400"></i>
                                </div>
                                @error('end_date')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            ...
                            @if($project->end_date)
                                ...
                                @endphp
                                <div class="mt-4 p-3 {{ $daysLeft < 0 ? 'bg-red-50' : ($daysLeft < 7 ? 'bg-yellow-50' : 'bg-green-50') }} rounded-xl">
                                    <div class="flex items-center gap-2 text-xs {{ $daysLeft < 0 ? 'text-red-700' : ($daysLeft < 7 ? 'text-yellow-700' : 'text-green-700') }}">
                                        <i class="fas {{ $daysLeft < 0 ? 'fa-exclamation-triangle' : 'fa-clock' }}"></i>
                                        @if($daysLeft < 0)
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <ul class="text-xs text-gray-600 space-y-1.5">
                                    <li class="flex items-center gap-2"><i class="fas fa-check-circle text-green-500 text-xs"></i> Keep project title clear and descriptive</li>
                                    <li class="flex items-center gap-2"><i class="fas fa-check-circle text-green-500 text-xs"></i> Update status as project progresses</li>
                                    <li class="flex items-center gap-2"><i class="fas fa-check-circle text-green-500 text-xs"></i> Add/remove modules as scope changes</li>
                                    <li class="flex items-center gap-2"><i class="fas fa-check-circle text-green-500 text-xs"></i> Adjust deadlines if needed</li>
                                </ul>
                            </div>
                        </div>
                    </div>
