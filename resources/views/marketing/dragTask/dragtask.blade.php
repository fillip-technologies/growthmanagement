...
@section('content')
                                ...
                                <div class="flex justify-between items-start gap-3">
                                    <div class="flex-1">
                                        ...
                                        <div class="mt-2 flex items-center gap-2 text-xs text-gray-500">
                                            ...
                                            <span>Deadline:
                                                {{ $task->end_date ? \Carbon\Carbon::parse($task->end_date)->format('d M Y') : 'Not set' }}</span>
                                        </div>
                                    </div>
                                    <button type="button"
                                        ...
                                        data-task-id="{{ $task->id }}" data-task-name="{{ $task->project_name }}">
                    ...
                    <div class="bg-white rounded-2xl shadow-xl overflow-hidden employee-card">
                        ...
                        <div id="employee-{{ $emp->id }}" data-employee-id="{{ $emp->id }}"
                            ...
                            @foreach ($assignedTasks as $astask)
                                @php
                                    $status = strtolower($astask->marketingpro->status);
                                    $statusColors = [
                                        'pending' => [
                                            'bg' => 'bg-yellow-100',
                                    ...
                                    $priority = strtolower($astask->marketingpro->priority);
                                    $priorityColors = [
                                        'low' => 'text-green-600',
                                    ...
                                    <div class="flex justify-between items-start gap-2">
                                        <div class="flex-1">
                                            <h3 class="font-semibold text-gray-800 text-sm flex items-center gap-1">
                                                ...
                                                {{ Str::limit($astask->marketingpro->project_name, 25) }}
                                            </h3>
                                        ...
                                        <button type="button"
                                            ...
                                            data-task-name="{{ $astask->marketingpro->name }}"
    ...
    <form id="assignForm"
        action="{{ route('mrk.assignDragTask') }}" method="POST" style="display: none;">
        @csrf
        @php
            $id = null;
